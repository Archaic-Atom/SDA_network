...
class BuildGraph(object):
    ...
    def RestoreModel(self):
        ...
        #                        if ('U-Net' not in v.name
                                    #and 'ExtractUnaryFeatureBlock3' not in v.name
    ...
    def __GenTrainingData(self, model, input, output, label, opt, training=True):
        if training == True:
            acc = model.Accuary(output, input)
            loss = model.Loss(output, input, label)
            grads = []
        ...
        with tf.device('/gpu:%d' % num):
            with tf.name_scope('%s_%d' % ('TOWER', num)):
                ...
                grads, loss, acc = self.__GenTrainingData(
                    model, tem_input, output, tem_label, opt, training)
    ...
    def __SummaryData(self, loss, acc):
        if loss != None:
            length = loss.get_shape().as_list()[0]
            # print length
            for i in range(length):
                name = "l" + str(i)
                tf.summary.scalar(name, loss[i])

        if acc != None:
            length = acc.get_shape().as_list()[0]
            for i in range(length):
                name = "a" + str(i)
                tf.summary.scalar(name, acc[i])
